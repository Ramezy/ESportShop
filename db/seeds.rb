...
admin = Admin.create( :name => "Aramide", :address => "295 Princess Street")

order = Order.create( :status => "Processing", :admin_id => admin, :cost => 20, :quantity => 5)
...
data.each do |cleat|
    ...
    pro_order = ProductOrder.create(pro["quantity"])
end
